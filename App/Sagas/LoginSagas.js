...
export function* login({email, password, alertfunc, nav}) {
  try {
    const response = yield call(
      dbService.auth.signInWithEmailAndPassword,
      email.toString(),
      password.toString(),
      function() {},
    );
...
export function* socialLogin({token, alertfunc, nav}) {
  try {
    ...
    const response = yield call(
      dbService.auth.signInWithCredential,
      token,
      function() {},
    );
    ...
    if (guardian) {
      ...
      resetAction = nav.reset(
        [NavigationActions.navigate({routeName: 'ScreenOne'}, {uid})],
        0,
      );
    }
    yield call(nav.dispatch, resetAction);

...
class Summary extends Component {
  ...
  render() {
    ...
    function dataToAvatar(children, childs, nav, bVisible) {
      return Object.keys(children).map((item, i) => {
          ...
          return (
            <View key={keyId} style={{padding: 5}}>
              <Avatar
                ...
                onEditPress={() =>
                  nav.navigate('EditChildScreen', {
                    childData: children[item],
                    ckey: item,
                  })
                }
    ...
    function dataToTag(items, cat) {
      return items.map((item, i) => {
        ...
        return (
          <Badge
            textStyle={{fontSize: 12, color: 'rgba(0,0,0,0.5)'}}
            value={item}
            status="error"
            key={keyId}
            badgeStyle={style.tagItem}
          />
        );
      });
    }
    ...
    const greetingCopy = greeting ? (
      ...
      <Text style={[style.summaryCopy, style.summaryBodyCopy]}>
        It looks like you don't have a summary bio yet, you can add one by
        clicking the edit button (which you don't see yet, because we're
        developing it).
      </Text>
    );
    return (
      <View style={style.container}>
          ...
          <View style={{flex: 0.2, paddingLeft: 5, alignItems: 'center'}}>
            <TouchableHighlight
              ...
              <View style={style.ellipsis}>
                <Text style={{width: 20, textAlign: 'center', color: '#fff'}}>
                  {ellipseText}
                </Text>
              </View>
            </TouchableHighlight>
        ...
        <UsersList
          addChildren={this.props.addChildrenStatus}
          data={arr}
          isCurrentUser={uid === payload.uid}
          title="CHILDREN"
          addNewChild={() => this.props.nav.navigate('CreateChildScreen')}
        />
      </View>
    );
  }
